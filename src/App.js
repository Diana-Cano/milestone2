...
function App() {
  ...
  useEffect(() => {
      const fetchCategoryData = async () => {
          const response = await fetch("https://buck-it-back-end.vercel.app/api/categories")
          const data = await response.json()
          setCategories(data);
      }
      fetchCategoryData();
      const fetchListData = async () => {
        const response = await fetch("https://buck-it-back-end.vercel.app/api/lists/category/all")
        const data = await response.json()
        setLists(data);
      }
      fetchListData();
